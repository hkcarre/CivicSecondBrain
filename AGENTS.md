...
### Three core operations (all call the Anthropic Codex API)

- **INGEST** (`app/lib/Codex/ingest-engine.ts`, `scripts/ingest-seed.ts`): Downloads a document, parses it (skipping files >25MB or non-PDF formats), calls Codex to extract `ExtractedKnowledge`, writes/updates wiki pages, saves manifest entry.
- **QUERY** (`app/api/chat/route.ts`): Reads `wiki/index.md`, keyword-selects relevant pages via `selectRelevantPages()`, streams a Codex response. Page selection is keyword-based — intended to be replaced with OpenSearch k-NN at scale.
- **LINT** (`app/api/lint/route.ts`, `scripts/lint-wiki.ts`): Reads topic wiki pages, sends to Codex for analysis, writes `wiki/recommendations/` pages. The script version reads only 8 fixed topic pages to bound memory; the API route version reads the full wiki.

### Key paths

- `app/lib/Codex/client.ts` — Anthropic SDK singleton + model constants + system prompts
